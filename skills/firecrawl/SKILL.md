...
This skill covers live web work during a session. Upstream Firecrawl also ships `firecrawl-build` skills (integrating Firecrawl into product code) and `firecrawl-workflows` skills (deep research, SEO audit, QA, lead generation, knowledge bases). **Neither is installed here** — only the CLI skills are vendored into this repo. Install them with `npx -y firecrawl-cli@latest init --all --browser` if a task needs them.
...
- **Integrating Firecrawl into an app, adding `FIRECRAWL_API_KEY` to `.env`, or choosing endpoint usage in product code** -> the `firecrawl-build` skills, not vendored here (`npx skills add firecrawl/skills`)
- **Producing Firecrawl-powered deliverables such as research briefs, SEO audits, QA reports, lead lists, knowledge bases, or design-system extraction** -> the `firecrawl-workflows` skills, not vendored here (`npx -y firecrawl-cli@latest init --all`)

...
**Preservation check (required for edit mode).** Re-read the rewrite against the original and confirm none of the following changed: fenced code blocks, YAML frontmatter, blockquotes, table cells, inline code, URLs, file paths, or heading structure. Also confirm the rewrite removed more flagged patterns than it introduced. If any of these changed, revert that specific change before delivering. Those are the promises made above; this is what checks them. Rewording a heading to fix Title Case and stripping an AI tracking parameter from a URL are carved out, because this skill instructs both.

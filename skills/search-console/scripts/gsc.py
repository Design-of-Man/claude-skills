#!/usr/bin/env python3
"""Google Search Console client — works across every client property.
...
unattended from a scheduled routine. See references/setup.md.
...
Never commit the key file. Client repos are often public.

ONE service account serves every client. Steps 1-3 of setup happen once, ever;
adding a client is only step 4 (paste the same client_email into their property).

Usage:
  python3 gsc.py sites                      # every property this key can read
  python3 gsc.py verify
  python3 gsc.py summary  --days 90
  python3 gsc.py queries  --days 90 --limit 50
  python3 gsc.py pages    --days 90 --limit 50
  python3 gsc.py compare  --days 90
  python3 gsc.py raw --dimensions query,page --days 28 --json
...
# No hardcoded client. Set GSC_PROPERTY per client, or pass --property.
# A bare nickname is resolved against the properties the key can read.
DEFAULT_PROPERTY = os.environ.get("GSC_PROPERTY")
...
def load_credentials(key_arg=None):
    ...
    sys.exit(
        ...
        "  Setup steps: references/setup.md"
    )
    ...
    try:
        ...
        if exc.code == 404:
            detail += (
                ...
                "`python3 gsc.py verify` to list the exact strings."
            )
        sys.exit(f"gsc: API error ({exc.code}) on {path}: {detail}")


def list_sites(token):
    return api_call(token, "/sites").get("siteEntry", [])


def resolve_property(token, wanted):
    """Turn a client nickname into the exact property string.

    'sc-domain:x.com' and 'https://x.com/' pass through. Anything else is
    matched as a substring against the properties this key can actually read,
    so `--property abacoa` works without anyone memorising the exact form.
    """
    sites = list_sites(token)
    urls = [e["siteUrl"] for e in sites]
    if not urls:
        sys.exit(
            "gsc: this service account cannot read any properties yet.\n"
            "  It authenticated fine, so the key is good — it just has not been added\n"
            "  as a user on any Search Console property. See references/setup.md step 4."
        )
    if not wanted:
        if len(urls) == 1:
            return urls[0]
        sys.exit(
            "gsc: --property is required when the key can read more than one.\n"
            "  Readable: " + ", ".join(urls) + "\n"
            "  Pass a full string or a nickname, e.g. --property " + urls[0].split(":")[-1][:12]
        )
    if wanted in urls:
        return wanted
    hits = [u for u in urls if wanted.lower() in u.lower()]
    if len(hits) == 1:
        return hits[0]
    if not hits:
        sys.exit(
            f"gsc: no readable property matches {wanted!r}.\n"
            "  Readable: " + ", ".join(urls) + "\n"
            "  If the client is missing, step 4 was not done for them."
        )
    sys.exit(f"gsc: {wanted!r} is ambiguous — matches " + ", ".join(hits))
...
def cmd_sites(args, token):
    ...
    if not entries:
        print(
            ...
            "The key authenticates fine — it just has not been added as a user on any\n"
            "Search Console property yet. That is step 4 in references/setup.md,\n"
            "and it is the step people skip."
        )
        return 1
    print(f"{len(entries)} propert{'y' if len(entries)==1 else 'ies'} readable by this key:\n")
    for e in sorted(entries, key=lambda x: x["siteUrl"]):
        print(f"  {e['siteUrl']}  [{e['permissionLevel']}]")
    print("\nAdd a client by pasting this key's client_email into their")
...
def cmd_verify(args, token):
    entries = list_sites(token)
    if not entries:
        return cmd_sites(args, token)
    print("Readable properties:")
    for e in sorted(entries, key=lambda x: x["siteUrl"]):
        marker = "  <- selected" if e["siteUrl"] == args.property else ""
        print(f"  {e['siteUrl']}  [{e['permissionLevel']}]{marker}")
    print(f"\nConnected. Try: python3 gsc.py summary --property {args.property} --days 90")
    return 0
...
def main():
    ap = argparse.ArgumentParser(
        description="Query Google Search Console for any client property.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="Setup: references/setup.md  |  Start with: gsc.py sites",
    )
    ap.add_argument("--key", help="path to the service-account JSON key")
    ap.add_argument("--property", default=DEFAULT_PROPERTY,
                    help="property, or a nickname matched against readable ones "
                         "(default: $GSC_PROPERTY, or the only readable one)")
    ...
    ap.add_argument("command",
                    choices=["sites", "verify", "summary", "queries", "pages",
                             "compare", "raw"])
    ...
    # Every other command needs a concrete property.
    args.property = resolve_property(token, args.property)
